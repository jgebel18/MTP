...
class PlottingFunctions:
    ...
    def Plot_Thermal_mapping(tri ,T, Nodes ,BC, Element_view):
        ...
        if (BC==f'Dirichlet'):
            ...
            plt.title('Stacionární teplotní pole s Newtonovou  okrajovou podmínkou a prouděním', fontsize=14, fontweight='bold')
        plt.xlabel('X [m]')
        plt.ylabel('Y [m]')
        plt.axis('equal')
        ...
        if (BC == f'Dirichlet'):
            ...
            plt.savefig(f'Thermal_mapping_Newton_Sationar-Jet.png')
        plt.show()
    ...
    def Plot_Thermal_mapping_time(tri ,T, Nodes , Element_view, time):
        ...
        plt.title(f'Teplotní pole Newtonovy okrajové úlohy s prouděním pro čas : {np.round(time/86400, 2)} dní', fontsize=14, fontweight='bold')
        plt.xlabel('X [m]')
        plt.ylabel('Y [m]')
        plt.axis('equal')
        plt.grid(True, linestyle=':', alpha=0.5)
        ...
        plt.savefig(f'Thermal_mapping_Newton_Sationar-time-{time}-Jet.png')
        plt.show()
    ...
    def GenerateThermalprofile_stationar( Nx,Nodes,T, BC,indicies):
        ...
        if (BC == f'Dirichlet'):
            ...
            plt.title(f'Teplotní profil stacionární Newtonovy úlohy s prouděním',
                      fontsize=14, fontweight='bold')
        ...
        if (BC == f'Dirichlet'):
            ...
            plt.savefig(f'Primka_Newton_Sationar-Jet.png')
        plt.show()
    @staticmethod
    def GenerateThermalProfileUnstationar( Nx,Nodes,T,indicies, Time):
        ...
        plt.title(f'Teplotní profil Newtonovy úlohy s prouděním pro čas: {np.round(Time/86400, 2)}',
                  fontsize=14, fontweight='bold')

        plt.grid(True, linestyle=':', alpha=0.5)

        plt.savefig(f'Primka_Newton_Sationar_time_{Time}-jet.png')
    ...
    def Generate_Time_Thermal_Dependence(T_vector, indicies, Nodes, Time):
        ...
        plt.title('Časová závislost teploty v zadaných bodech v rámci Newtonovy nestacionární úlohy s prouděním',
                  fontsize=14, fontweight='bold')

        plt.savefig('Time_dependence-Newton-jet.png')
        plt.show()

    @staticmethod
    def Plot_Thermal_mapping_time_with_jets(tri, T, Nodes, Element_view, time, grid_vx,grid_vy, grid_x, grid_y):
        plt.figure(figsize=(12, 8))
        # Vykreslení teplotního pole na trojúhelníkové síti
        tcf = plt.tricontourf(tri, T, levels=100, cmap='jet')
        cb = plt.colorbar(tcf)
        ...
        plt.streamplot(grid_x, grid_y, grid_vx, grid_vy, density=1.4, linewidth=None, color='white')
        # Příkaz triplot vykreslí samotné čáry trojúhelníků
        # 'g-' znamená zelená (green) plná čára. Můžeš dát 'b-' pro modrou, 'k-' pro černou atd.
        if Element_view == True:
            plt.triplot(triang, 'g-', linewidth=0.5, alpha=0.7)
        ...
        plt.title(f'Teplotní pole Newtonovy okrajové úlohy s prouděním pro čas : {np.round(time / 86400, 2)} dní', fontsize=14,
                  fontweight='bold')
        plt.xlabel('X [m]')
        plt.ylabel('Y [m]')
        plt.axis('equal')
        ...
        plt.savefig(f'Thermal_mapping_Newton_Sationar-time-{time}-jets-field-li.png')
        plt.show()
